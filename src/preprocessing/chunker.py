...
def main(args):
    ...
    out_df = pd.DataFrame(
        unique_rows,
        columns=["web_id", "chunk_id", "title", "url", "kind", "text"]
    )
    out_df.to_csv(args.output, index=False)
    print(f"Сохранено {len(out_df)} чанков в {args.output}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Чанкинг websites_updated.xlsx → chunks.csv")
    parser.add_argument(
        "--input",
        default="data/raw/websites_updated.xlsx",
        help="Путь к исходному XLSX"
    )
    parser.add_argument(
        "--output",
        default="data/processed/chunks.csv",
        help="Путь к результату"
    )
